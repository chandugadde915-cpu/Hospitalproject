...
const LOCAL_USERS = [
  { id: "HA-JANATHA-001", username: "janathaadmin@janathahospitals.local", email: "janathaadmin@janathahospitals.local", name: "Janatha Hospitals Admin", role: "HOSPITAL_ADMIN", jobRole: "Hospital Admin", hospitalId: "janatha-hospital", hospitalCode: "JANATHA", hospitalName: "Janatha Hospitals", accessScope: "ALL_BRANCHES", status: "Active" },
...
function localRequest(path, options = {}) {
  ...
  if (path === "/auth/login" && method === "POST") {
    ...
    const baseUser = LOCAL_USERS.find((entry) => [entry.email, entry.username].filter(Boolean).some((value) => String(value).toLowerCase() === login));
    const expectedPassword = baseUser?.id === "HA-JANATHA-001" ? JANATHA_ADMIN_PASSWORD : LOCAL_PASSWORD;
